<?php
    class appointments extends database {
        ...
        function listAppointmentsByPatients() {
            $sql = $this->database->query('SELECT `appointments.id`, `appointments.dateHour`, `patients.lastname`, `patients.firstname` FROM `appointments` INNER JOIN `patients` ON `appointments.idPatients` = `patients.id` WHERE `appointments.idPatients` = `patients.id`');
            return $sql->fetchAll(PDO::FETCH_OBJ);
        }
    }
?>